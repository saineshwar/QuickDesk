$(document).ready(function () {
    $('#GridTableContainer').jtable({
        ...
        actions:
        {
            listAction: '/HODDashboard/GetAllTickets',
            deleteAction: '/HODDashboard/DeleteTicket'
        },
    ...
    $("#Username").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/HODDashboard/GetAllAgentUsers",
                type: "GET",
                dataType: "json",
...
function Ticket(trackingId) {
    window.location.href = '/TicketDetailsHod/Details?TrackingId=' + trackingId;
    return false;
}
...
function processChangeAllPriority(values) {
    $.ajax({
        ...
        url: "/HODDashboard/ProcessChangeAllPriority",
...
function ProcessChangeAllStatus(values) {
    $.ajax({
        type: "POST", //HTTP POST Method
        url: "/HODDashboard/ProcessChangeAllStatus",
...
function AssignUserTicket() {
    ...
    if ($selectedRows.length > 0) {
        ...
        if (values != "") {
            $.ajax({
                type: "POST", //HTTP POST Method
                url: "/HODDashboard/AssignTickettoUser",
