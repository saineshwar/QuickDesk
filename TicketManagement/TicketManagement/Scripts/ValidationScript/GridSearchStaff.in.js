$(document).ready(function () {
    $('#GridTableContainer').jtable({
        ...
        actions:
        {
            listAction: '/AgentDashboard/GetAllTickets',
            deleteAction: '/AgentDashboard/DeleteTicket'
        },
            ...
            Priority:
            {
                ...
                display: function (data) {
                    if (data.record.Priority == "Low") {
                        ...
                        return '<span class="label status status-closed" style="color: #EF7215;"> <strong>' + data.record.Priority + '</strong></span>';
                    }
                    else if (data.record.Priority == "Urgent") {
                        return '<span class="label status status-closed" style="color: #ff5959;"> <strong>' + data.record.Priority + '</strong></span>';
                    }
                }
            },
            Status: {
    ...
    $("#Username").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/AgentDashboard/GetAllAgentMembers",
                type: "GET",
                dataType: "json",
...
function Ticket(trackingId) {
    window.location.href = '/TicketDetailsAgent/Details?TrackingId=' + trackingId;
    return false;
}
...
var url = "/AgentDashboard/GetAllSearchFields";
...
var url = "/AgentDashboard/GetAllPriority";
$.getJSON(url, function (data) {
    if (data) {
        $("#DropPriority").empty();
...
var url = "/AgentDashboard/GetAllStatus";
$.getJSON(url, function (data) {
    if (data) {
        $("#DropStatus").empty();
...
function processChangeAllPriority(values) {
    $.ajax({
        ...
        url: "/AgentDashboard/ProcessChangeAllPriority",
...
function ProcessChangeAllStatus(values) {
    $.ajax({
        type: "POST", //HTTP POST Method
        url: "/AgentDashboard/ProcessChangeAllStatus",
...
function AssignUserTicket() {
    ...
    if ($selectedRows.length > 0) {
        ...
        if (values != "") {
            $.ajax({
                type: "POST", //HTTP POST Method
                url: "/AgentDashboard/AssignTickettoUser",
